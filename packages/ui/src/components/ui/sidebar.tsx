...
function SidebarProvider({
  ...
  // Internal state of the sidebar. openProp/setOpenProp allow control from outside.
  ...
  // Exposed as data-state="expanded" | "collapsed" for CSS targeting.
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-0.5 hover:after:bg-sidebar-border sm:flex',
...
function SidebarMenuSkeleton({
  ...
  // useState's lazy initializer is guaranteed to run exactly once, unlike
  // useMemo's cache (which React may discard and recompute) - required
  // here since the value is intentionally random.
